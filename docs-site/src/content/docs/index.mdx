...
| Feature | Description |
|---------|-------------|
| **Transcription** | Whisper-based transcription with Google Drive integration, parallel workers, and checkpoint-aware batch processing |
| ⚖️ **Judging** | Separate `judge-transcription` and `judge-qa` commands with per-record `JudgePipelineResult` persistence |
| 🧠 **CEP QA Generation** | Bloom's Taxonomy-scaffolded QA pairs with downstream judge integration |
| 🕸️ **KG Construction** | AutoSchemaKG-powered entity/relation extraction with GraphML export |
