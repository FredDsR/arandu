#!/bin/bash
# =============================================================================
# Arandu Transcription Judge Common Job Script
#
# Shared logic for all transcription-judge SLURM scripts. Source this from
# partition-specific scripts; do not run directly.
#
# Required environment variables (set by partition scripts):
#   (none — defaults cover everything)
#
...
#   ARANDU_JUDGE_LANGUAGE           - Expected transcription language (default: pt)
...
# Validator settings — default to the local Ollama sidecar. Passing these
...
# PIPELINE_ID is required — the judge runs against a specific pipeline's
# transcription outputs.
: "${PIPELINE_ID:?PIPELINE_ID env var is required (e.g. 'PIPELINE_ID=test-cep-01 sbatch ...')}"
export PIPELINE_ID

INPUT_DIR="$ARANDU_RESULTS_DIR/$PIPELINE_ID/transcription/outputs"

# -----------------------------------------------------------------------------
# Job Information
# -----------------------------------------------------------------------------
echo "=============================================="
echo "Arandu Transcription Judge Job Started"
echo "=============================================="
...
if [ ! -d "$INPUT_DIR" ]; then
    echo "Error: Transcription outputs not found at $INPUT_DIR"
    echo "Run transcription for pipeline '$PIPELINE_ID' first."
    exit 1
fi
...
# -----------------------------------------------------------------------------
echo ""
echo "Starting transcription-judge process..."
echo "=============================================="

docker compose -f "$COMPOSE_FILE" --profile "$DOCKER_PROFILE" \
    up arandu-judge --abort-on-container-exit
...
echo ""
echo "=============================================="
echo "Arandu Transcription Judge Job Completed"
echo "=============================================="
echo "End Time:       $(date)"
echo "Judgements:     $INPUT_DIR/../judgements.json"
