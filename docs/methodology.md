...
**Grounding scope.** The source text the judge evaluates against is the **chunk the pair was generated from**, not the whole transcription. Generation runs the Bloom ladder independently per chunk (Section 4.3), so a document-scoped judge could accept a pair on evidence sitting in some other part of the interview -- evidence the generator never saw. That is precisely the local hallucination the faithfulness gate exists to catch, so the judge is given the same window as the generator. The source-metadata block (participant, researcher, location, date, event context) is appended to that chunk whenever generation injected it, keeping the two sides symmetric on metadata as well as on text.
