...
def build_kg(
    ...
    no_concepts: Annotated[
        ...
        typer.Option(
            "--no-concepts",
            help="Skip concept generation (only extract triples).",
        ),
    ] = False,
    backend_option: Annotated[
        ...
        typer.Option(
            "--backend-option",
            help="Backend-specific option as KEY=VALUE (repeatable).",
        ),
    ] = None,
    pipeline_id: Annotated[
        str | None,
    ...
    Examples:
        ...
        gtranscriber build-kg results/ --provider ollama -m qwen3:14b -l pt

        # Skip concept generation for faster processing
        gtranscriber build-kg results/ --no-concepts

        # Pass backend-specific options
        gtranscriber build-kg results/ --backend-option chunk_size=4096

        # Use a specific pipeline ID
        gtranscriber build-kg results/ --id my-pipeline
    """
