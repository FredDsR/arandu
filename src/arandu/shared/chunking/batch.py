...
class ChunkBatchConfig(BaseModel):
    ...
    input_dir: str = Field(..., description="Source directory of TranscriptionRecord JSON files.")
...
class ChunkBatchResult(BaseModel):
    ...
    ``sources_processed`` is the unit-of-work count (one TranscriptionRecord =
...
def run_chunk_batch(
    ...
    Args:
        input_dir: Directory of ``TranscriptionRecord`` JSON files.
        ...
        rebuild: When ``True``, clear any existing chunk outputs for the
            requested views and reset the checkpoint before processing, so a
            re-run yields a clean view (used to drop chunks of transcriptions
            the transcription judge later rejected).
    ...
    if not json_files:
        logger.info("No TranscriptionRecord JSON files found in %s", input_dir)
    ...
    for json_file in json_files:
        try:
            record = TranscriptionRecord.model_validate_json(json_file.read_text())
        except ValidationError as exc:
            logger.debug("Skipping non-TranscriptionRecord file %s: %s", json_file, exc)
            skipped += 1
            continue
